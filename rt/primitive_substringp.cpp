...
void primitive_substringp(lpp::lpp__context::pointer_t context)
{
    ...
    case lpp::lpp__value_type_t::LPP__VALUE_TYPE_WORD:
        if(!thing1->represents_word())
        {
            throw lpp::lpp__error(context
                                ...
                                , "error"
                                , "\"substringp\" first parameter must be a word when the second parameter is a word.");
        }
    ...
    case lpp::lpp__value_type_t::LPP__VALUE_TYPE_LIST:
        {
            lpp::lpp__value::pointer_t result(std::make_shared<lpp::lpp__value>(false));
            context->set_return_value(result);
        }
        break;
    ...
    default:
        throw lpp::lpp__error(context
                            , lpp::lpp__error_code_t::ERROR_CODE_INVALID_DATUM
                            , "error"
                            , "\"substringp\" used with an unexpected second parameter type.");
