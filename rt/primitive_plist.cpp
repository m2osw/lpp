...
void primitive_plist(lpp::lpp__context::pointer_t context)
{
    ...
    || thing->get_value() == nullptr)
    {
        throw lpp::lpp__error(context
                            , lpp::lpp__error_code_t::ERROR_CODE_INVALID_DATUM
                            , "error"
                            , "\"plistp\" named \"" + listname->to_word() + "\" is not defined.");
    }
    ...
    if(value->type() != lpp::lpp__value_type_t::LPP__VALUE_TYPE_PROPERTY_LIST)
    {
        throw lpp::lpp__error(context
                            , lpp::lpp__error_code_t::ERROR_CODE_INVALID_DATUM
                            , "error"
                            , "somehow the value of \"plistp\" named \"" + listname->to_word() + "\" is not a property list.");
    }

    lpp::lpp__value::vector_t list;
    lpp::lpp__value::map_t const & prop(value->get_prop());
    for(auto const & p : prop)
    {
        list.push_back(std::make_shared<lpp::lpp__value>(p.first));
